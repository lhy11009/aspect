...
#include <aspect/material_model/rheology/peierls_creep1.h>
...
namespace aspect
{
  namespace MaterialModel
  {
    ...
    {
      template <int dim>
      PeierlsCreep1<dim>::PeierlsCreep1 ()
        = default;
       ...
       */
      template <int dim>
      const PeierlsCreep1Parameters
      PeierlsCreep1<dim>::compute_creep_parameters (const unsigned int composition,
                                                    const std::vector<double> &phase_function_values,
                                                    const std::vector<unsigned int> &n_phase_transitions_per_composition) const
      {
        PeierlsCreep1Parameters creep_parameters;
        // todo_pcut
      ...
      template <int dim>
      double
      PeierlsCreep1<dim>::compute_approximate_viscosity (const double strain_rate,
                                                         ...
                                                         const std::vector<unsigned int> &n_phase_transitions_per_composition) const
      {
        ...
        const PeierlsCreep1Parameters p = compute_creep_parameters(composition, phase_function_values, n_phase_transitions_per_composition);
      ...
      template <int dim>
      double
      PeierlsCreep1<dim>::compute_exact_viscosity (const double strain_rate,
                                                   const double pressure,
                                                   const double temperature,
                                                   const unsigned int composition,
                                                   const std::vector<double> &phase_function_values,
      ...
      template <int dim>
      double
      PeierlsCreep1<dim>::compute_exact_viscosity_TwoD (const double strain_rate,
                                                        const double pressure,
                                                        const double temperature,
                                                        const unsigned int composition,
                                                        const std::vector<double> &phase_function_values,
      ...
      template <int dim>
      double
      PeierlsCreep1<dim>::compute_viscosity (const double strain_rate,
                                             const double pressure,
                                             const double temperature,
                                             const unsigned int composition,
                                             const std::vector<double> &phase_function_values,
      ...
      template <int dim>
      double
      PeierlsCreep1<dim>::compute_viscosity_TwoD (const double strain_rate,
                                                  const double pressure,
                                                  const double temperature,
                                                  const unsigned int composition,
                                                  const std::vector<double> &phase_function_values,
      ...
      PeierlsCreep1<dim>::compute_approximate_strain_rate_and_derivative (const double stress,
                                                                          const double pressure,
                                                                          const double temperature,
                                                                          const PeierlsCreep1Parameters creep_parameters) const
      {
        /**
        * b = (E+P*V)/(R*T)
        * c = std::pow(gamma, p)
        ...
        */
        const PeierlsCreep1Parameters p = creep_parameters;
      ...
      template <int dim>
      std::pair<double, double>
      PeierlsCreep1<dim>::compute_exact_strain_rate_and_derivative (const double stress,
                                                                    const double pressure,
                                                                    const double temperature,
                                                                    const PeierlsCreep1Parameters creep_parameters) const
      {
      ...
      template <int dim>
      std::pair<double, double>
      PeierlsCreep1<dim>::compute_strain_rate_and_derivative (const double stress,
                                                              const double pressure,
                                                              const double temperature,
                                                              const PeierlsCreep1Parameters creep_parameters) const
      {
      ...
      template <int dim>
      void
      PeierlsCreep1<dim>::declare_parameters (ParameterHandler &prm)
      {
      ...
      template <int dim>
      void
      PeierlsCreep1<dim>::parse_parameters (ParameterHandler &prm,
                                            const std::unique_ptr<std::vector<unsigned int>> &expected_n_phases_per_composition)
      {
        ...
        options.property_name = "Peierls shear modulus";
        shear_moduluses = Utilities::MapParsing::parse_map_to_double_array(prm.get("Peierls shear modulus"),
                                                                           options);


        options.property_name = "Peierls shear modulus derivative";
        shear_modulus_derivatives = Utilities::MapParsing::parse_map_to_double_array(prm.get("Peierls shear modulus derivative"),
                                                                                     options);
        ...
        reference_temperatures = Utilities::MapParsing::parse_map_to_double_array(prm.get("Reference temperatures"),
                                                                                  options);

        options.property_name = "Reference pressures";
        reference_pressures = Utilities::MapParsing::parse_map_to_double_array(prm.get("Reference pressures"),
                                                                               options);
      }
    }
  }
...
#define INSTANTIATE(dim) \
  namespace Rheology \
  { \
    template class PeierlsCreep1<dim>; \
  }
