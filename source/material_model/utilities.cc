            ...
            if (is_using_metastable_kinetics)
              {
                ...
                for (unsigned int i=0; i<n_phase_transitions_per_composition->size(); ++i)
                  {
                    if ((in.phase_index >= start_phase_index) && (in.phase_index < start_phase_index + (*n_phase_transitions_per_composition)[i] + 1))
                      {
                        composition_index = i;
                        break;
                      }
                ...
                for (unsigned int phase_index_temp = start_phase_index; phase_index_temp <= in.phase_index; phase_index_temp++)
                  {
                    if (use_metastable_kinetics[phase_index_temp])
                      {
                        beyond_metastable_transition = true;
                        break;
                      }
                  }
                if (beyond_metastable_transition)
                  {
                    function_value *= 1.0 - (*in.composition)[metastable_index];
                    // function_value = 0.0;
                  }
              }
          }
        ...
        if (use_depth_instead_of_pressure)
          {
            ...
            compute_latent_heats        = Utilities::parse_map_to_double_array (prm.get("Compute latent heat"),
                                                                                ...
                                                                                n_phase_transitions_per_composition,
                                                                                true);
            std::vector<double> use_metastable_kinetics_double     = Utilities::parse_map_to_double_array (prm.get("Metastable transition"),
                                                                     list_of_composition_names,
                                                                     has_background_field,
                                                                     "Whether use metastable kinetics on phases",
                                                                     true,
                                                                     n_phase_transitions_per_composition,
                                                                     true);
            ...
            std::vector<double>  metastable_comp_indexes_double    = Utilities::parse_map_to_double_array (prm.get("Metastable transition comp"),
                                                                     list_of_composition_names,
                                                                     has_background_field,
                                                                     "Metastable transition compositional index",
                                                                     true,
                                                                     n_phase_transitions_per_composition,
                                                                     true);
            metastable_comp_indexes.resize(metastable_comp_indexes_double.size());
            std::transform(metastable_comp_indexes_double.begin(), metastable_comp_indexes_double.end(), metastable_comp_indexes.begin(),
                           [](double val)
            {
...
// Fix the kinetics model
      void MO_KINETICS::setKineticsFixed(double P, double T, double Coh)
      {
        ...
        Y_func = [this, P, T, P_eq, Coh](double t)
        {
          return Y_func_ori(P, T, P_eq, Coh);
        };
        I_func = [this, P, T, P_eq](double t)
        {
          return I_func_ori(P, T, P_eq);
        };
      }
...
// Set phase transition equilibrium
        ...
        bool debug)
      {
        ...
        if (!is_saturated)
          {
            auto it = std::find_if(s_values.begin(), s_values.end(), [s_saturation](double s)
            {
              return s > s_saturation;
            });
