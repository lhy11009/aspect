...
namespace aspect
{
  namespace MaterialModel
  {
    namespace EquationOfState
    {
      template <int dim>
      void
      ThermodynamicTableLookupMorb<dim>::initialize()
      {
        ...
        for (unsigned i = 0; i < n_material_lookups; ++i)
          {
            if (material_file_format == perplex)
              {
                material_lookup
                .push_back(std::make_unique<MaterialModel::MaterialUtilities::Lookup::PerplexReaderMorb>(data_directory+material_file_names[i],
      ...
      template <int dim>
      unsigned int
      ThermodynamicTableLookupMorb<dim>::number_of_lookups() const
      {
      ...
      template <int dim>
      bool
      ThermodynamicTableLookupMorb<dim>::
      ...
      fill_phase_volume_fractions (const MaterialModel::MaterialModelInputs<dim> &in,
                                   const std::vector<std::vector<double>> &volume_fractions,
                                   PhaseFractionAdditionalOutputs<dim> *phase_volume_fractions_out,
                                   const unsigned morb_composition_index) const
      {
        ...
        for (unsigned int i = 0; i < in.n_evaluation_points(); ++i)
          for (unsigned int k = 0; k < unique_phase_indices[0].size(); ++k)
            phase_volume_fractions[unique_phase_indices[0][k]][i] += volume_fractions[i][morb_composition_index] * material_lookup[0]->phase_volume_fraction(k,in.temperature[i],in.pressure[i]);
        ...
        for (unsigned int i=0; i < in.n_evaluation_points(); ++i)
          {
            ...
            // eclogite phase fraction
            eos_outputs[i].eos_eclogite_phase_volume_fraction  = material_lookup[0]->phase_volume_fraction(0,in.temperature[i],in.pressure[i]);
          }
      ...
      template <int dim>
      void
      ThermodynamicTableLookupMorb<dim>::fill_additional_outputs (const MaterialModel::MaterialModelInputs<dim> &in,
                                                                  const std::vector<std::vector<double>> &volume_fractions,
                                                                  MaterialModel::MaterialModelOutputs<dim> &out,
                                                                  unsigned morb_composition_index) const
      {
        ...
        if (PhaseFractionAdditionalOutputs<dim> *phase_volume_fractions_out = out.template get_additional_output<PhaseFractionAdditionalOutputs<dim>>())
          fill_phase_volume_fractions(in, volume_fractions, phase_volume_fractions_out, morb_composition_index);
      ...
      template <int dim>
      void
      ThermodynamicTableLookupMorb<dim>::declare_parameters (ParameterHandler &prm)
      {
      ...
      template <int dim>
      void
      ThermodynamicTableLookupMorb<dim>::parse_parameters (ParameterHandler &prm)
      {
      ...
      template <int dim>
      void
      ThermodynamicTableLookupMorb<dim>::create_additional_named_outputs (MaterialModel::MaterialModelOutputs<dim> &out) const
      {
        if (out.template get_additional_output<PhaseFractionAdditionalOutputs<dim>>() == nullptr)
          {
            const unsigned int n_points = out.n_evaluation_points();
            out.additional_outputs.push_back(
              std::make_unique<MaterialModel::PhaseFractionAdditionalOutputs<dim>> (unique_phase_names, n_points));
          }
        /*
      ...
      ThermodynamicTableLookupMorb<dim>::get_material_lookup (unsigned int lookup_index) const
      {
...
#define INSTANTIATE(dim) \
  template class ThermodynamicTableLookupMorb<dim>;
