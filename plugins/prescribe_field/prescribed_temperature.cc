...
namespace aspect
{
  ...
  double plate_model_T1_3d(const Point<3> &p, const double depth)
  {
      // plate model T1 in 3d: identical to the function in 2d
      // Use plate model to compute the temperature, migrated from the world builder
      // in order to generate consistent result with the world builder.
      // Fix the overiding plate side by creating a minor ridge there.
      const double x = p[0];
      ...
      for (int i = 1; i<sommation_number+1; ++i)
        {
          ...
          else if (x > x_extent - overiding_plate_area_width - side_distance)
            {
              // for the overiding plate
              age = (x_extent - side_distance - x) / overiding_plate_area_width * overiding_plate_age;
              temperature = temperature + (bottom_temperature_local - top_temperature) *
                          ((2 / (double(i) * M_PI)) * std::sin((double(i) * M_PI * depth) / max_depth) *
                           std::exp((((overriding_plate_pseudo_velocity * max_depth)/(2 * thermal_diffusivity)) -
                                     std::sqrt(((overriding_plate_pseudo_velocity*overriding_plate_pseudo_velocity*max_depth*max_depth) /
  ...
  // todo_chunk
  double plate_model_T1_3d_chunk(const Point<3> &p, const double depth)
  {
      // plate model T1 in 3d: identical to the function in 2d
      // Use plate model to compute the temperature, migrated from the world builder
      // in order to generate consistent result with the world builder.
      // Fix the overiding plate side by creating a minor ridge there.
      const double phi = std::atan2(p[1], p[0]); // this is the angle along longitude
      const double l = outer_radius*phi;
      const double l_extent = outer_radius*phi_M_rad;
      const double thermal_diffusivity = 1e-6;
      const double thermal_expansion_coefficient = 3e-5;
      const double gravity_norm = 10.0;
      const double specific_heat = 1250.0;
      ...
      for (int i = 1; i<sommation_number+1; ++i)
        {
          ...
          else if (l > l_extent - overiding_plate_area_width - side_distance)
            {
              // for the overiding plate
              age = (l_extent - side_distance - l) / overiding_plate_area_width * overiding_plate_age;
              temperature = temperature + (bottom_temperature_local - top_temperature) *
                          ((2 / (double(i) * M_PI)) * std::sin((double(i) * M_PI * depth) / max_depth) *
                           std::exp((((overriding_plate_pseudo_velocity * max_depth)/(2 * thermal_diffusivity)) -
                                     std::sqrt(((overriding_plate_pseudo_velocity*overriding_plate_pseudo_velocity*max_depth*max_depth) /
      ...
      for (int i = 1; i<sommation_number+1; ++i)
        {
          // suming over the "sommation_number"
          // use a spreading ridge around the left corner and a pseudo spreading ridge around the right corner 
          if (outer_radius * phi < area_width)
          {
          ...
          else if (outer_radius * (phi_M_rad - phi) < overiding_plate_area_width)
            {
              const double dist = outer_radius * (phi_M_rad - phi);
              // for the overiding plate
    ...
    prm.enter_subsection ("Prescribed temperatures");
    {
      prm.enter_subsection ("Indicator function");
      {
        prm.declare_entry ("Coordinate system", "cartesian",
                           ...
                           "are interpreted as r,phi or r,phi,phi in 2D/3D "
                           "respectively with phi being the polar angle. `depth' "
      ...
      prm.enter_subsection ("Plate model 1");
      {
        ...
        prm.declare_entry("Side distance", "0.0",
                          Patterns::Double (0.),
                        ...
                        ParameterHandler &prm)
  {
    ...
    prm.enter_subsection ("Geometry model");
    {
      ...
      prm.enter_subsection ("Chunk");
      {
          ...
          phi_M = prm.get_double("Chunk maximum longitude");
          phi_M_rad = phi_M * M_PI / 180.0;
      }
      prm.leave_subsection();
    }
    prm.leave_subsection();
    ...
    prm.enter_subsection ("Prescribed temperatures");
    {
      ...
      else if (temperature_model == "plate model 1")
      {
          ...
          prm.enter_subsection ("Plate model 1");
          {
            side_distance = prm.get_double("Side distance");
            area_width = prm.get_double("Area width");
            subducting_plate_velocity = prm.get_double("Subducting plate velocity");
            overiding_plate_age = prm.get_double("Overiding plate age");
                        ...
                        if (dim == 2)
                        ...
                        else
                          {
                            ...
                            if (temperature_model == "function"){
                              ...
                              // use the plate model1
                              const Point<3> p3d = as_3d(Utilities::convert_array_to_point<dim>(function_point.get_coordinates()));
                              const double depth = simulator_access.get_geometry_model().depth(p);
                              if (indicator > 0.5)
                              {
                                if (geometry_model_name == "box")
                                  u_i       = plate_model_T1_3d(p3d, depth);
                                else if (geometry_model_name == "chunk")
                                  u_i       = plate_model_T1_3d_chunk(p3d, depth); //todo_chunk
                                else
                                {
                                  AssertThrow (false, ExcMessage ("The prescribed_temperature plugin in 3d only works"
                                                  "with the geometry of box\n\n"));
                                }
                              }
                            }
                          }
