...
public:
    ...
    double operator() (double, bool) const;
...
double spline::operator() (double x, bool force_linear_extrapolation=false) const
{
    size_t n=m_x.size();
    ...
    if(force_linear_extrapolation){
        interpol = (x - m_x[idx])/(m_x[idx+1] - m_x[idx]) * m_y[idx+1] +\
                   (x - m_x[idx+1])/(m_x[idx] - m_x[idx+1]) * m_y[idx]; 
    }
    else{
        if(x<m_x[0]) {
