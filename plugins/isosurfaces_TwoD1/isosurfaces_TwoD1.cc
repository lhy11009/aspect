...
namespace aspect
{
  namespace MeshRefinement
  {
    ...
    {
      bool
      IsosurfaceTwoD1::are_all_values_in_range(const std::vector<double> &values) const
      {
    ...
    template <int dim>
    void
    IsosurfacesTwoD1<dim>::update ()
    { }

    template <int dim>
    void
    IsosurfacesTwoD1<dim>::tag_additional_cells () const
    {
      if (this->get_dof_handler().n_locally_owned_dofs() == 0)
        return;

      const int maximum_refinement_level = this->get_parameters().initial_global_refinement +
              ...
              for (unsigned int i_quad=0; i_quad<quadrature.size(); ++i_quad)
                {
                  ...
                  const double depth = this->get_geometry_model().depth(i_point); // this for the coarsening in the lower mantle
    ...
    template <int dim>
    void
    IsosurfacesTwoD1<dim>::
    declare_parameters (ParameterHandler &prm)
    {
      prm.enter_subsection("Mesh refinement");
      {
        prm.enter_subsection("IsosurfacesTwoD1");
        {
          ...
          prm.declare_entry ("Depth for coarsening the lower mantle", "700e3",
                             Patterns::Double(),
                             "A depth for coarsening material in the lower mantle");
          prm.declare_entry ("Level for coarsening the lower mantle", "0",
                             Patterns::Integer(),
                             "A level for coarsening material in the lower mantle. By default, this is 0, thus no coarsening is applied");
    ...
    template <int dim>
    void
    IsosurfacesTwoD1<dim>::parse_parameters (ParameterHandler &prm)
    {
      prm.enter_subsection("Mesh refinement");
      {
        ...
        prm.enter_subsection("IsosurfacesTwoD1");
        {
          ...
          for (auto &isosurface_entry : isosurface_entries)
            {
              ...
              aspect::MeshRefinement::internal::IsosurfaceTwoD1 isosurface;
          ...
          coarsen_lm_depth = prm.get_double("Depth for coarsening the lower mantle");
          coarsen_lm_level = prm.get_integer("Level for coarsening the lower mantle");
        }
        prm.leave_subsection();
      }
      prm.leave_subsection();
...
namespace aspect
{
  namespace MeshRefinement
  {
    ASPECT_REGISTER_MESH_REFINEMENT_CRITERION(IsosurfacesTwoD1,
                                              "isosurfaces_twod1",
