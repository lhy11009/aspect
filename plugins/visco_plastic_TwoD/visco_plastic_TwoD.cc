...
namespace aspect
{
  namespace MaterialModel
  {
                 ...
                 const SymmetricTensor<2,dim> &strain_rate) const
    {
      ...
      const std::vector<double> volume_fractions = MaterialUtilities::compute_composition_fractions(composition, get_volumetric_composition_mask());
    ...
    is_yielding(const MaterialModelInputs<dim> &in) const
    {
      ...
      const std::vector<double> volume_fractions = MaterialUtilities::compute_composition_fractions(in.composition[0], get_volumetric_composition_mask());
      ...
      */
      ...
      const std::pair<std::vector<double>, std::vector<bool>> calculate_viscosities =
        calculate_isostrain_viscosities(in, 0, volume_fractions, viscous_flow_law,
                                        ...
                                        phase_function.n_phase_transitions_for_each_chemical_composition());
    ...
    template <int dim>
    PlasticTwoDAdditionalOutputs<dim>::PlasticTwoDAdditionalOutputs (const unsigned int n_points)
      :
    ...
    template <int dim>
    std::vector<double>
    PlasticTwoDAdditionalOutputs<dim>::get_nth_output(const unsigned int idx) const
    {
    ...
    template <int dim>
    TwoDAdditionalViscosityOutputs<dim>::TwoDAdditionalViscosityOutputs (const unsigned int n_points)
      :
      ...
      for (unsigned int j=0; j < volume_fractions.size(); ++j)
        {
          ...
          double viscosity_diffusion;
          if (use_phase_rheology_mixing)
            {
              if (in.metastable_grain_sizes && in.metastable_grain_sizes->size()>0 && !std::isnan((*in.metastable_grain_sizes)[i]))
                {
                  // with a metastable grain size, and it must be initiated with values
                  // the grain size is invalid if it's not initiated by the MaterialInputs class.
                  // it iss nan if it's not processed by the kinetics.
                  double metastable_grain_size = (*in.metastable_grain_sizes)[i];
                  auto is_metastable_phase = (*in.is_metastable_phases);
                  viscosity_diffusion = diffusion_creep.compute_viscosity_mixing(pressure_for_creep, temperature_for_viscosity, j,
                                                                                 re_phase_function_values,
                                                                                 phase_function.n_phase_transitions_for_each_chemical_composition(),
                                                                                 phase_rheology_mixing_models[j],
                                                                                 minimum_viscosity_for_composition,
            ...
            {
              viscosity_diffusion = diffusion_creep.compute_viscosity(pressure_for_creep, temperature_for_viscosity, j,
                                                                      re_phase_function_values,
                                                                      phase_function.n_phase_transitions_for_each_chemical_composition());
            }
          ...
          double viscosity_dislocation;
          if (use_phase_rheology_mixing)
            {
              viscosity_dislocation = dislocation_creep.compute_viscosity_mixing(edot_ii, pressure_for_creep, temperature_for_viscosity, j,
                                                                                 re_phase_function_values,
                                                                                 phase_function.n_phase_transitions_for_each_chemical_composition(),
                                                                                 phase_rheology_mixing_models[j],
                                                                                 minimum_viscosity_for_composition,
            ...
            {
              viscosity_dislocation = dislocation_creep.compute_viscosity(edot_ii, pressure_for_creep, temperature_for_viscosity, j,
                                                                          re_phase_function_values,
                                                                          phase_function.n_phase_transitions_for_each_chemical_composition());
            }
             ...
             MaterialModel::MaterialModelOutputs<dim> &out) const
    {
      ...
      // todo_chemical
      std::vector<std::vector<double>> all_volume_fractions (in.n_evaluation_points(), std::vector<double> (this->introspection().n_chemical_composition_fields()+1));
      ...
      // remake the inputs for viscosity modules
      // include entries from metastable kinetics
      MaterialModel::MaterialModelInputs<dim> in_visc = in;

      std::vector<bool> is_metastable_phases(phase_function.n_phases(), false);
      for (unsigned int phase=0; phase<phase_function.n_phases(); ++phase)
        {
          is_metastable_phases[phase] = phase_function.is_metastable_phase(phase);
        }
      in_visc.fill_metastable_phases(is_metastable_phases);
      ...
      if (this->introspection().compositional_name_exists("meta_grain_size"))
        {
          ...
          unsigned int meta_grain_size_index = this->introspection().compositional_index_for_name("meta_grain_size");
          unsigned int grain_density_index = this->introspection().compositional_index_for_name("meta_x0");

          std::vector<double> meta_grain_sizes = std::vector<double>(in_visc.n_evaluation_points(), std::numeric_limits<double>::quiet_NaN());
          for (unsigned int i=0; i < in_visc.n_evaluation_points(); ++i)
            {
              const double metastable = in_visc.composition[i][metastable_index];
              const double grain_density = in_visc.composition[i][grain_density_index];

              if (metastable > metastable_threshold_for_grain_growth && grain_density > grain_density_threshold_for_grain_growth)
                {
                  meta_grain_sizes[i] = in_visc.composition[i][meta_grain_size_index];
                }
            }
          in_visc.fill_metastable_grain_sizes(meta_grain_sizes);
        }
      ...
      for (unsigned int i=0; i < in.n_evaluation_points(); ++i)
        {
          ...
          if (use_lookup_table_morb)
            {
              ...
              const std::vector<unsigned int> &n_phase_transitions_per_composition = phase_function.n_phase_transitions_for_each_chemical_composition();
              // First step: get the volume fraction of the eclogite phase composition
              ...
              // Second step: apply cutoff values
              // smaller values are cutoff
              // high P, T regions are fixed to 1.0
              double phase_function_value;
              if (eclogite_phase_volume_fraction < eclogite_phase_cutoff_below)
                phase_function_value = 0.0;
              else if ((in.temperature[i] > eclogite_phase_cutoff_above_T1) && (in.pressure[i] > eclogite_phase_cutoff_above_P1))
                phase_function_value = 1.0;
              else if ((in.temperature[i] > eclogite_phase_cutoff_above_T2) && (in.pressure[i] > eclogite_phase_cutoff_above_P2))
                phase_function_value = 1.0;
              else
                phase_function_value = eclogite_phase_volume_fraction;

              // Final step: re-calibrate the function values with eclogite_phase_volume_fraction_divisor
              // and limit the value in (0.0, 1.0)
              phase_function_for_rheology_values[start_phase_index + 0 -morb_composition_index] =
                std::min(std::max(phase_function_value / eclogite_phase_volume_fraction_divisor, 0.0), 1.0);
            }
          ...
          //  out.entropy_derivative_pressure[i] and out.entropy_derivative_temperature[i] above are both 0.0
          {
            ...
            double entropy_gradient_metastable = 0.0;
              ...
              for (unsigned int phase=0; phase<phase_function.n_phase_transitions(); ++phase)
                {
                  ...
                  int base = 0;
                  const std::vector<unsigned int> &n_phases_per_composition = phase_function.n_phase_transitions_for_each_chemical_composition();
                  for (unsigned int j = 0; j < volume_fractions.size(); j++)
                    {
                  ...
                  if (this->introspection().compositional_name_exists("meta_rate"))
                    {
                      const double PhaseFunction = phase_function.compute_value1(phase_in);
                      unsigned int meta_rate_index = this->introspection().compositional_index_for_name("meta_rate");
                      const double meta_rate = in.composition[i][meta_rate_index];
                      entropy_gradient_metastable += meta_rate * PhaseFunction * entropy_change;
                    }
                }
            ...
            out.entropy_derivative_metastable[i] = entropy_gradient_metastable;
          }


          // Compute the effective viscosity if requested and retrieve whether the material is plastically yielding
          bool plastic_yielding = false;
          if (in_visc.requests_property(MaterialProperties::viscosity))
            {
              ...
              if (const std::shared_ptr<TwoDAdditionalViscosityOutputs<dim>> add_viscosities_out = out.template get_additional_output_object<TwoDAdditionalViscosityOutputs<dim>>())
                {
                  ...
                  calculate_viscosities = calculate_isostrain_viscosities(in_visc, i, volume_fractions, viscous_flow_law,
                                                                          ...
                                                                          phase_function.n_phase_transitions_for_each_chemical_composition(), add_viscosities_out);
                }
              else
                {
                  calculate_viscosities = calculate_isostrain_viscosities(in_visc, i, volume_fractions, viscous_flow_law,
          ...
          fill_plastic_outputs(i,volume_fractions,plastic_yielding,in,out,\
                               phase_function_values, phase_function.n_phase_transitions_for_each_chemical_composition());
      ...
      strain_rheology.compute_finite_strain_reaction_terms(in_visc, out);


      if (use_elasticity)
        {
          elastic_rheology.fill_elastic_additional_outputs(in_visc, average_elastic_shear_moduli, out);
          elastic_rheology.fill_reaction_outputs(in_visc, average_elastic_shear_moduli, out);
        }
      ...
      if (reset_viscosity)
        {
          for (unsigned int i=0; i < in_visc.n_evaluation_points(); ++i)
            {
              reset_calculated_viscosities(i, out.viscosities, in_visc);
            }
        }
      ...
      if (reset_composition_viscosity)
        {
          for (unsigned int i=0; i < in.n_evaluation_points(); ++i)
            {
              reset_calculated_composition_viscosities(i, out.viscosities, in_visc);
            }
        }
      ...
      if (reaction_metastable && in.compute_metastable_reaction)
        {
          for (unsigned int i=0; i < in.n_evaluation_points(); ++i)
            {
              const std::shared_ptr<ReactionRateOutputs<dim>> reaction_rate_out = out.template get_additional_output_object<ReactionRateOutputs<dim>>();
    ...
    template <int dim>
    void
    ViscoPlasticTwoD<dim>::declare_parameters (ParameterHandler &prm)
    {
      prm.enter_subsection("Material model");
      {
        prm.enter_subsection ("Visco Plastic TwoD");
        {
          ...
          Rheology::PeierlsCreep1<dim>::declare_parameters(prm);
          ...
          prm.enter_subsection ("Lookup table morb");
          {
            ...
            prm.declare_entry ("Eclogite phase divisor", "1.0", Patterns::Double(),
                               "Divisor for the eclogite phase");
          ...
          prm.declare_entry ("Use phase rheology mixing", "false", Patterns::Bool(),
                             "Use phase rheology mixing");

          prm.declare_entry ("Phase rheology mixing models", "0", Patterns::List(Patterns::Integer()),
                             "Phase rheology mixing models");

          // Entries for mestastable phase change
          ...
          prm.declare_entry ("Reaction metastable trivial", "false",
                             Patterns::Bool (),
                             "Whether to assign trivial values to compute the metastable reaction"
                             "if true, reaction terms will be assigned 0.0 values");
          prm.declare_entry ("Metastable water contents", "150.0",
                             Patterns::Double(),
                             "Amount of Coh in the metastable kinetics");

          prm.enter_subsection("Metastable kinetics");
          {
            // MaterialUtilities::MO_KINETICS<dim>::declare_parameters(prm);
    ...
    template <int dim>
    void
    ViscoPlasticTwoD<dim>::parse_parameters (ParameterHandler &prm)
    {
      ...
      prm.enter_subsection("Material model");
      {
        prm.enter_subsection ("Visco Plastic TwoD");
        {
          ...
          options.property_name = "Minimum viscosity";
          min_visc = Utilities::MapParsing::parse_map_to_double_array(prm.get("Minimum viscosity"), options);

          options.property_name = "Maximum viscosity";
          max_visc = Utilities::MapParsing::parse_map_to_double_array(prm.get("Maximum viscosity"), options);
          ...
          if (use_peierls_creep)
            {
              peierls_creep = std::make_unique<Rheology::PeierlsCreep1<dim>>();
          ...
          prm.enter_subsection ("Lookup table morb");
          {
            ...
            eclogite_phase_volume_fraction_divisor = prm.get_double("Eclogite phase divisor");
          }
          prm.leave_subsection();
          ...
          if (use_phase_rheology_mixing)
            AssertThrow(phase_rheology_mixing_models.size() == n_fields,
                        ExcMessage("Size of lookup indexes has to match number of fields + 1 "));

          // Entries for metastable phase transition
          reaction_metastable = prm.get_bool("Reaction metastable");
          reaction_metastable_trivial = prm.get_bool("Reaction metastable trivial");
          coh_for_Kinetics = prm.get_double("Metastable water contents");
          if (reaction_metastable)
            {
              Mo_Kinetics = std::make_unique<MaterialUtilities::MO_KINETICS<dim>>();
              Mo_Kinetics->initialize();
              prm.enter_subsection("Metastable kinetics");
              {
                Mo_Kinetics->parse_parameters(prm);
              }
              prm.leave_subsection();
    ...
    template <int dim>
    void
    ViscoPlasticTwoD<dim>::reset_calculated_viscosities( const unsigned int i,
                                                                      ...
                                                                      const MaterialModel::MaterialModelInputs<dim> &in) const
    {
      ...
      if ((depth < sz_embeded_fault.sz_depth) &&
          (this->get_postprocess_manager().template
           has_matching_active_plugin<const Postprocess::Particles<dim>>()))
        {
          ...
          double dist;
          bool is_first = true;
          const Postprocess::Particles<dim> &particle_postprocessor = this->get_postprocess_manager().template get_matching_active_plugin<Postprocess::Particles<dim>>();
          const dealii::Particles::ParticleHandler<dim> &particle_handler = particle_postprocessor.get_particle_manager(0).get_particle_handler();
    ...
    template <int dim>
    double
    ViscoPlasticTwoD<dim>::computeEqP(const double T) const
    {
      return Mo_Kinetics->computeEqP(T);
    }
                                                            ...
                                                            const MaterialModel::MaterialModelInputs<dim> &in) const
    {
      ...
      // not a valid cell or at the 0th step
      if  ((in.current_cell.state() != IteratorState::valid) || this->get_timestep_number() == 0)
        return;

      // test performing a trivial step
      if (reaction_metastable_trivial)
        return;

      // configure the Mo kinetic variables
      // const double Coh = 150.0;
      Mo_Kinetics->setKineticsFixed(in.pressure[i], in.temperature[i], coh_for_Kinetics);
      const double P_eq = Mo_Kinetics->computeEqP(in.temperature[i]);

      // if equilibrium is not reached, return 0.0 values
      if (in.pressure[i] < P_eq)
        return;
      ...
      const double X_is_old = composition[metastable_index + 5];
      ...
      const bool is_saturated_old = (X_is_old > 0.99);
      ...
      const double X_is = result_last[6];
      ...
      reaction_terms[i][metastable_index+5] = X_is - X_is_old;
      if (this->introspection().compositional_name_exists("meta_rate"))
        reaction_terms[i][metastable_index+6] = result_last[7];
    ...
    EclogiteDecoupledViscosity<dim>::declare_parameters (ParameterHandler &prm)
    {
      ...
      prm.enter_subsection ("Eclogite decoupled viscosity");
      {
        ...
        prm.declare_entry ("Crust index", "1", Patterns::Integer (0),
                           "Index of the crust");
      }
      prm.leave_subsection();
    ...
    EclogiteDecoupledViscosity<dim>::parse_parameters (ParameterHandler &prm)
    {
      ...
      prm.enter_subsection ("Eclogite decoupled viscosity");
      {
        ...
        comp_crust = prm.get_integer("Crust index");
      }
      prm.leave_subsection();
    }
                              ...
                              const std::vector<unsigned int> &n_phases_per_composition) const
    {
      ...
      for (unsigned i=0; i<comp_crust; ++i)
...
namespace aspect
{
  namespace MaterialModel
  {
    ASPECT_REGISTER_MATERIAL_MODEL(ViscoPlasticTwoD,
                                   "visco plastic twod",
