      ...
      if (reaction_metastable)
        {
          ...
          const bool include_derivative=1;

          // Initialize the MO_KINETICS class
          Mo_Kinetics = std::make_unique<MaterialUtilities::MO_KINETICS>(d0, A, n, dS, dV, dHa, V_star_growth, fs, Vm,
                                                                         gamma, K0, nucleation_type, include_derivative);
          ...
          if (in.requests_property(MaterialProperties::viscosity))
            {
              ...
              if (MaterialModel::MaterialModelDerivatives<dim> *derivatives =
                    out.template get_additional_output<MaterialModel::MaterialModelDerivatives<dim>>())
                compute_viscosity_derivatives(i, volume_fractions, calculate_viscosities.first, in, out, phase_function_values, phase_function.n_phase_transitions_for_each_chemical_composition());
            }
